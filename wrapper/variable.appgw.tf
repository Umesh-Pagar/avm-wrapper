...
DESCRIPTION
  ...
  validation {
    ...
    condition     = alltrue([for _, v in var.agw_backend_http_settings : v.pick_host_name_from_backend_address == true ? v.host_name == null : true])
...
variable "agw_gateway_ip_configuration" {
  type = object({
...
variable "agw_name" {
  ...
  validation {
    condition     = var.agw_name == null || can(regex("^[a-z0-9-]{3,24}$", var.agw_name))
...
variable "agw_waf_policy_resource_id" {
  type        = string
  default     = null
  description = "(Optional) The ID of the Web Application Firewall Policy."
}

variable "agw_authentication_certificate" {
  type = map(object({
    data = string
    name = string
  }))
...
variable "agw_autoscale_configuration" {
  type = object({
...
variable "agw_create_public_ip" {
  type        = bool
  default     = true
  description = "Optional public IP to auto create public id"
}

variable "agw_custom_error_configuration" {
  type = map(object({
    custom_error_page_url = string
    status_code           = string
  }))
...
variable "agw_diagnostic_settings" {
  type = map(object({
...
DESCRIPTION
  ...
  validation {
    condition     = alltrue([for _, v in var.agw_diagnostic_settings : contains(["Dedicated", "AzureDiagnostics"], v.log_analytics_destination_type)])
    error_message = "Log analytics destination type must be one of: 'Dedicated', 'AzureDiagnostics'."
  }
  validation {
    condition = alltrue(
      [
        for _, v in var.agw_diagnostic_settings :
...
variable "agw_fips_enabled" {
  type        = bool
  default     = null
  description = "(Optional) Is FIPS enabled on the Application Gateway?"
}

variable "agw_frontend_ip_configuration_private" {
  type = object({
    name                            = optional(string)
...
variable "agw_frontend_ip_configuration_public_name" {
  type        = string
  default     = null
...
variable "agw_private_link_configuration" {
  type = set(object({
    name = string
...
variable "agw_probe_configurations" {
  type = map(object({
    name                                      = string
...
variable "agw_public_ip_name" {
  type        = string
  default     = null
  description = "The name of the public IP resource. If not supplied, will be inferred from the resource name."

  validation {
    # Check if public_ip_name is null or matches the regex for a valid name
    condition     = var.agw_public_ip_name == null || can(regex("^[a-z0-9-]{3,80}$", var.agw_public_ip_name))
...
DESCRIPTION
  ...
  validation {
    condition     = can(regex("^(Standard_v2|WAF_v2)$", var.agw_sku.name))
    error_message = "SKU name must be 'Standard_v2' or 'WAF_v2'."
  }
  validation {
    condition     = can(regex("^(Standard_v2|WAF_v2)$", var.agw_sku.tier))
...
variable "agw_trusted_client_certificate" {
  type = map(object({
    data = string
    name = string
  }))
...
variable "agw_trusted_root_certificate" {
  type = map(object({
    data                = optional(string)
    key_vault_secret_id = optional(string)
    name                = string
...
variable "agw_url_path_map_configurations" {
  type = map(object({
    name                                = string
...
variable "agw_waf_configuration" {
  type = object({
    enabled                  = bool
