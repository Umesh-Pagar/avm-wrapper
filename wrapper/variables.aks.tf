...
variable "aks_name" {
  type        = string
  description = "The name of the Kubernetes cluster. Must be between 1 and 63 characters long and can only contain lowercase letters, numbers and hyphens."
  default     = null

  validation {
    condition     = var.aks_name == null || can(regex("^[a-zA-Z0-9]([a-zA-Z0-9\\-_]{0,61}[a-zA-Z0-9])?$", var.name))
...
variable "aks_sku_tier" {
  ...
  validation {
    condition     = can(index(["Free", "Standard", "Premium"], var.aks_sku_tier))
